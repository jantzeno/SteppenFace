...
class PlateArrangementManager:
    ...
    def set_packing_mode(self, mode: str):
        """
        Set the packing mode.

        Args:
            mode: One of "nfp", "rectangle"
        """
        if mode in ["nfp", "rectangle"]:
            self.packing_mode = mode
            logger.info(f"Packing mode set to '{mode}'")
        else:
            logger.warning(
                f"Unknown packing mode '{mode}', keeping '{self.packing_mode}'"
            )

    def set_nfp_quality(self, quality: str):
        """
        Set the nfp calculation quality

        Args:
            quality: One of "high", "medium", "fast"
        """
        if quality in ["high", "medium", "fast"]:
            self.nfp_quality = quality
            logger.info(f"NFP quality set to '{quality}'")
        else:
            logger.warning(
