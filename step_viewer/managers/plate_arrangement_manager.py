...
class PlateArrangementManager:
    ...
    def set_packing_strategy(self, strategy: str):
        ...
        if strategy in ["best_fit", "first_fit", "bottom_left"]:
            ...
            logger.warning(f"Unknown packing strategy '{strategy}', keeping '{self.packing_strategy}'")

    def arrange_parts_on_plates(self, parts_list: List, display=None) -> List[PackingResult]:
        """
        ...
        logger.info(f"Settings: spacing={self.spacing_mm}mm, rotation={'enabled' if self.allow_rotation else 'disabled'}")
        ...
        for rect in rectangles:
            ...
            if not fits_normal and not fits_rotated:
                oversized_parts.append(f"Part {rect.part_idx}: {rect.width:.1f}x{rect.height:.1f}mm")

        if oversized_parts:
            logger.error(f"Cannot arrange: {len(oversized_parts)} part(s) too large for plate ({plate_width:.1f}x{plate_height:.1f}mm):")
        ...
        for idx, rect in enumerate(rectangles):
            placed = False
            logger.info(f"Packing part {rect.part_idx} ({idx+1}/{len(rectangles)}): {rect.width:.1f}x{rect.height:.1f}mm")
            ...
            for plate in self.plate_manager.plates:
                ...
                if placement:
                    ...
                    placed = True
                    logger.info(f"  -> Placed on {plate.name} at ({x:.1f}, {y:.1f}), rotation={rotation:.2f}rad, size=({placed_width:.1f}x{placed_height:.1f}mm)")
                    logger.info(f"      Occupies: X=[{x:.1f}, {x+placed_width:.1f}], Y=[{y:.1f}, {y+placed_height:.1f}]")
                    break

            # If not placed on any existing plate, create a new one
            if not placed:
                new_plate = self.plate_manager.add_plate(f"Plate {self.plate_manager.next_plate_id}")
                logger.info(f"  -> All existing plates full, created {new_plate.name}")

                # Try to place on new plate
                placement = self._find_placement_on_plate(rect, new_plate, packing_results)
                ...
                if placement:
                    ...
                    packing_results.append(PackingResult(
                        part_idx=rect.part_idx,
                    ...
                    logger.info(f"  -> Placed on {new_plate.name} at ({x:.1f}, {y:.1f}), rotation={rotation:.2f}rad")
                else:
                    logger.error(f"Could not place part {rect.part_idx} even on new plate (should not happen - oversized check failed?)")

        logger.info(f"Arrangement complete: {len(packing_results)} parts placed on {len(self.plate_manager.plates)} plate(s)")

        # Log statistics per plate
        for plate in self.plate_manager.plates:
            if plate.part_indices:
                utilization = self._calculate_plate_utilization(plate, packing_results)
                logger.info(f"  {plate.name}: {len(plate.part_indices)} parts, {utilization:.1f}% utilization")
    ...
    def _extract_part_rectangles(self, parts_list: List, display=None) -> List[Rectangle]:
        """
        ...
        for part_idx, (solid, color, ais_shape) in enumerate(parts_list):
            try:
                ...
                logger.debug(f"Part {part_idx}: bbox after transform: X=[{xmin:.1f}, {xmax:.1f}], Y=[{ymin:.1f}, {ymax:.1f}] -> {width:.1f}x{height:.1f}mm")
                ...
                if width > 0 and height > 0:
                    ...
                    logger.warning(f"Part {part_idx} has invalid dimensions ({width}x{height}), skipping")
    ...
    def _find_placement_on_plate(self, rect: Rectangle, plate,
                                  existing_placements: List[PackingResult]) -> Optional[Tuple[float, float, float]]:
        """
        ...
        logger.debug(f"Trying to place part {rect.part_idx} ({rect.width:.1f}x{rect.height:.1f}mm) on plate {plate.id}")
        ...
        if self.allow_rotation and abs(rect.width - rect.height) > 0.1:  # Don't rotate squares
        ...
        for width, height, rotation in orientations:
            ...
            placement = self._find_best_fit_placement(width, height, rotation, plate, plate_placements)
            ...
            if placement:
                ...
                if score < best_score:
                    ...
                    logger.debug(f"  Found placement: ({x:.1f}, {y:.1f}) rotation={rotation:.2f}, score={score:.1f}")
    ...
    def _find_best_fit_placement(self, width: float, height: float, rotation: float,
                                  plate, placements: List[PackingResult]) -> Optional[Tuple[float, float, float]]:
        """
        ...
        candidates = [(self.margin_mm, self.margin_mm)]  # Start with margin-aware origin
        ...
        for p in placements:
            ...
            candidates.append((px + pw + self.spacing_mm, py + ph + self.spacing_mm))  # Top-right
        ...
        for zone in plate.exclusion_zones:
            ...
            candidates.append((zone_right + self.spacing_mm, max(self.margin_mm, zone_bottom)))  # Right of zone
            candidates.append((max(self.margin_mm, zone_left - width - self.spacing_mm), max(self.margin_mm, zone_bottom)))  # Left of zone
            candidates.append((max(self.margin_mm, zone_left), zone_top + self.spacing_mm))  # Above zone
            candidates.append((max(self.margin_mm, zone_left), max(self.margin_mm, zone_bottom - height - self.spacing_mm)))  # Below zone
    ...
    def _is_valid_placement(self, x: float, y: float, width: float, height: float,
                            plate, placements: List[PackingResult]) -> bool:
        """
        ...
        if x < self.margin_mm or y < self.margin_mm or x + width > plate.width_mm - self.margin_mm or y + height > plate.height_mm - self.margin_mm:
            logger.debug(f"    Rejected: Out of bounds with margin ({x:.1f}, {y:.1f}) + ({width:.1f}x{height:.1f}) > plate ({plate.width_mm:.1f}x{plate.height_mm:.1f}) with {self.margin_mm:.1f}mm margin")
            return False
        ...
        for zone in plate.exclusion_zones:
            ...
            no_overlap = (x + width <= zone_x or
                         x >= zone_x + zone_w or
                         y + height <= zone_y or
                         y >= zone_y + zone_h)

            if not no_overlap:
                logger.debug(f"    Rejected: Within {self.margin_mm:.1f}mm margin of exclusion zone at ({zone.x:.1f}, {zone.y:.1f})")
                return False
        ...
        for p in placements:
            ...
            no_overlap = (x + width <= px_with_spacing or  # New rect is completely to the left
                         x >= px_with_spacing + pw_with_spacing or  # New rect is completely to the right
                         y + height <= py_with_spacing or  # New rect is completely below
                         y >= py_with_spacing + ph_with_spacing)  # New rect is completely above

            if not no_overlap:
                logger.debug(f"    Rejected: Overlaps with part {p.part_idx}")
                logger.debug(f"      New rect: [{x:.1f}, {x+width:.1f}] x [{y:.1f}, {y+height:.1f}]")
                logger.debug(f"      Existing: [{px_with_spacing:.1f}, {px_with_spacing+pw_with_spacing:.1f}] x [{py_with_spacing:.1f}, {py_with_spacing+ph_with_spacing:.1f}]")
                return False

        return True

    def _get_placed_rectangle_dimensions(self, placement: PackingResult) -> Tuple[float, float, float, float]:
        """
    ...
    def _calculate_plate_utilization(self, plate, placements: List[PackingResult]) -> float:
        """
    ...
    def apply_arrangement(self, parts_list: List, packing_results: List[PackingResult], display):
        """
        ...
        for result in packing_results:
            ...
            if not plate:
                logger.warning(f"Plate {result.plate_id} not found for part {result.part_idx}")
                continue

            # Get current transformation (from planar alignment)
            current_trsf = ais_shape.LocalTransformation() if ais_shape.HasTransformation() else gp_Trsf()

            # Get the TRANSFORMED bounding box (shape in current position)
            from OCC.Core.BRepBuilderAPI import BRepBuilderAPI_Transform
            transformed_solid = BRepBuilderAPI_Transform(solid, current_trsf, False).Shape()
            bbox = Bnd_Box()
            ...
            current_xmin, current_ymin, current_zmin, current_xmax, current_ymax, current_zmax = bbox.Get()
            ...
            logger.info(f"Part {result.part_idx}: current=({current_x:.1f}, {current_y:.1f}), size=({current_width:.1f}x{current_height:.1f}mm)")
            logger.info(f"Part {result.part_idx}: plate_offset=({plate.x_offset:.1f}, {plate.y_offset:.1f}), placement=({result.x:.1f}, {result.y:.1f})")
            logger.info(f"Part {result.part_idx}: target=({target_x:.1f}, {target_y:.1f}), needs rotation={abs(result.rotation) > 0.01}")
            ...
            if abs(result.rotation) > 0.01:
                logger.info(f"  Applying 90° rotation (rotation={result.rotation:.2f}rad)")
                ...
                rotation_trsf.SetRotation(gp_Ax1(gp_Pnt(center_x, center_y, current_z), gp_Dir(0, 0, 1)), result.rotation)
                ...
                rotated_solid = BRepBuilderAPI_Transform(solid, working_trsf, False).Shape()
                rotated_bbox = Bnd_Box()
                brepbndlib.Add(rotated_solid, rotated_bbox, True)

                rotated_xmin, rotated_ymin, rotated_zmin, rotated_xmax, rotated_ymax, rotated_zmax = rotated_bbox.Get()
                ...
                logger.info(f"  After rotation: position=({current_x:.1f}, {current_y:.1f}), size=({current_width:.1f}x{current_height:.1f}mm)")
            ...
            final_transformed_solid = BRepBuilderAPI_Transform(solid, final_trsf, False).Shape()
            final_bbox = Bnd_Box()
            brepbndlib.Add(final_transformed_solid, final_bbox, True)
            final_xmin, final_ymin, final_zmin, final_xmax, final_ymax, final_zmax = final_bbox.Get()
            ...
            logger.info(f"  Part {result.part_idx} VERIFY: actual position=({final_x:.1f}, {final_y:.1f}), size=({final_width:.1f}x{final_height:.1f}mm)")
            if abs(final_x - target_x) > 0.5 or abs(final_y - target_y) > 0.5:
                logger.warning(f"  Part {result.part_idx} MISMATCH: target was ({target_x:.1f}, {target_y:.1f}) but ended at ({final_x:.1f}, {final_y:.1f})")
